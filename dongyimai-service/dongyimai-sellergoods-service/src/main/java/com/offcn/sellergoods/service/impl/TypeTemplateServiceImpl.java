...
public class TypeTemplateServiceImpl extends ServiceImpl<TypeTemplateMapper,TypeTemplate> implements TypeTemplateService {
    ...
    public List<Map> findSpecList(Long typeId) {
        //获取模板对象
        TypeTemplate template = this.getById(typeId);
        //获取规格信息(json字符串)
        String specIdsJSON = template.getSpecIds();
        List<Map> list = JSON.parseArray(specIdsJSON,Map.class); // [{id:1,text:xxx,"options":[{},{},{}]}]
        for (Map m : list) {
            Integer specId = (Integer) m.get("id");
            LambdaQueryWrapper<SpecificationOption> wrapper = new LambdaQueryWrapper<>();
            wrapper.eq(SpecificationOption::getSpecId,Long.valueOf(specId));
            List<SpecificationOption> options = specificationOptionService.list(wrapper);
